...
# Caleb Kitzman, Grant Truesdale, Blake Murray, Asher Titus, Blake Johnson
# 11/16/2021
# Data for Project
...
# Analysis 4
finaldata3 <- group_by(finaldata, Runtime)
LengthEarnings <- summarise(finaldata3, Movies=n(), Min=min(GrossEarnings), Average=mean(GrossEarnings), Median=median(GrossEarnings), Max=max(GrossEarnings))
LengthEarnings
cor(finaldata3$Runtime, finaldata3$GrossEarnings)
finaldata3$LengthType <- cut(finaldata3$Runtime, c(0,100,150,250), c("Short", "Medium", "Long"))
lengthtable <- tapply(finaldata3$GrossEarnings, finaldata3$LengthType, FUN = mean)
lengthtable

barplot(lengthtable,
        names = names(lengthtable),
        ylab = "Gross Earnings",
        xlab = "Length Type",
        col = "Red")

mean(finaldata3$Runtime)

library(ggplot2)
qplot(Runtime,GrossEarnings,data = finaldata3,geom = "point",xlab = "Runtime", ylab = "Gross Earnings")

#Summary Statistics
summary(finaldata$GrossEarnings)
hist(finaldata$GrossEarnings, main = "Gross Earnings Distribution")
shapiro.test(finaldata$GrossEarnings)
